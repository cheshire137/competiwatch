...
class MatchExporterTest < ActiveSupport::TestCase
  ...
  test 'generated CSV can be imported' do
    ...
    importer = MatchImporter.new(oauth_account: @oauth_account, season: @season)
    assert_no_difference ['Match.count', 'Friend.count', 'MatchFriend.count'] do
      importer.import(path)
      assert_empty importer.errors
    end

    match1 = @oauth_account.matches.in_season(@season).find_by_rank(1234)
    refute_nil match1, 'should have recreated match1'
    assert_nil match1.map

    match2 = @oauth_account.matches.in_season(@season).find_by_rank(1254)
    ...
    match3 = @oauth_account.matches.in_season(@season).find_by_rank(1273)
    ...
    match4 = @oauth_account.matches.in_season(@season).find_by_rank(1295)
